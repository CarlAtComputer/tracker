 ...
 * @generated
 */
public class TrackerDiagramEditorUtil {
	...
	*/
	public static boolean openDiagram(Resource diagram) throws PartInitException {
		...
		page.openEditor(new URIEditorInput(diagram.getURI()), TrackerDiagramEditor.ID);
		return true;
	}
	...
	*/
	...
	public static Resource openModel(Shell shell, String description, TransactionalEditingDomain editingDomain) {
		...
		try {
			...
			TrackerDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + uri, we); //$NON-NLS-1$
			MessageDialog.openError(shell, Messages.TrackerDiagramEditorUtil_OpenModelResourceErrorDialogTitle,
					NLS.bind(Messages.TrackerDiagramEditorUtil_OpenModelResourceErrorDialogMessage, fileName));
		}
		return resource;
	}
	...
	public static void runWizard(Shell shell, Wizard wizard, String settingsKey) {
		IDialogSettings pluginDialogSettings = TrackerDiagramEditorPlugin.getInstance().getDialogSettings();
	...
	*/
	public static Resource createDiagram(URI diagramURI, URI modelURI, IProgressMonitor progressMonitor) {
		...
		progressMonitor.beginTask(Messages.TrackerDiagramEditorUtil_CreateDiagramProgressTask, 3);
		final Resource diagramResource = editingDomain.getResourceSet().createResource(diagramURI);
		final Resource modelResource = editingDomain.getResourceSet().createResource(modelURI);
		final String diagramName = diagramURI.lastSegment();
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(editingDomain,
				Messages.TrackerDiagramEditorUtil_CreateDiagramCommandLabel, Collections.EMPTY_LIST) {
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
					throws ExecutionException {
				Tracker model = createInitialModel();
				attachModelToResource(model, modelResource);

				Diagram diagram = ViewService.createDiagram(model, TrackerEditPart.MODEL_ID,
						TrackerDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
				if (diagram != null) {
				...
				try {
					modelResource.save(tracker.diagram.part.TrackerDiagramEditorUtil.getSaveOptions());
					diagramResource.save(tracker.diagram.part.TrackerDiagramEditorUtil.getSaveOptions());
				} catch (IOException e) {

					TrackerDiagramEditorPlugin.getInstance().logError("Unable to store model and diagram resources", e); //$NON-NLS-1$
				}
	...
	* @generated
	*/
	private static Tracker createInitialModel() {
		return TrackerFactory.eINSTANCE.createTracker();
	}
	...
	* @generated
	*/
	private static void attachModelToResource(Tracker model, Resource resource) {
