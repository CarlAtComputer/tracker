...
public class FilesystemCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	*/
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(FilesystemPackage.eINSTANCE.getFilesystem_Folders());
			myFeaturesToSynchronize.add(FilesystemPackage.eINSTANCE.getFilesystem_Files());
		}
	...
	*/
	protected void refreshSemantic() {
		...
		for (Iterator<FilesystemNodeDescriptor> descriptorsIterator = childDescriptors.iterator(); descriptorsIterator
				.hasNext();) {
			FilesystemNodeDescriptor next = descriptorsIterator.next();
			String hint = FilesystemVisualIDRegistry.getType(next.getVisualID());
	...
	*/
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<FilesystemLinkDescriptor> linkDescriptors = collectAllLinks(getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = FilesystemVisualIDRegistry.getVisualID(nextDiagramLink);
			...
			for (Iterator<FilesystemLinkDescriptor> linkDescriptorsIterator = linkDescriptors
				...
				FilesystemLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator.next();
	...
	* @generated
	*/
	private Collection<FilesystemLinkDescriptor> collectAllLinks(View view, Domain2Notation domain2NotationMap) {
		if (!FilesystemEditPart.MODEL_ID.equals(FilesystemVisualIDRegistry.getModelID(view))) {
		...
		case FilesystemEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(FilesystemDiagramUpdater.getFilesystem_1000ContainedLinks(view));
			}
		...
		case FolderEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(FilesystemDiagramUpdater.getFolder_2001ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case FileEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(FilesystemDiagramUpdater.getFile_2002ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	* @generated
	*/
	private Collection<IAdaptable> createConnections(Collection<FilesystemLinkDescriptor> linkDescriptors,
		...
		for (FilesystemLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					FilesystemVisualIDRegistry.getType(nextLinkDescriptor.getVisualID()), ViewUtil.APPEND, false,
